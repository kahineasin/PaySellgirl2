...
@Deprecated
public class PFMonthTaskBase<TTask>
...
//    {
//    	return PFDataHelper.ObjectToDateString(Calendar.getInstance());
//    }
    protected String GetCmonth()
    {
    	return SGDataHelper.DateToCMonth(SGDate.Now());
    }
    /// <summary>
    /// a上次执行的年月日(有成员是为了重设时间是清空)
    /// </summary>
...
//    private String finishedMsg="";
     ...
     */
    public PFMonthTaskBase(String hashId, PFFunc3<String, SGDate, TTask,Boolean> doAction, int backupDay,int backupHour, int backupMinute,boolean jumpThePassedTask,TaskFollower[] follower)
    {
...
//    }
	...
	protected SGDate GetBackupDay(SGDate now) {
		//return new PFDate(now.GetYear(), now.GetMonth(), now.GetDay(), BackupHour, BackupMinute, 0);
		return new SGDate(now.GetYear(), now.GetMonth(), BackupDay, BackupHour, BackupMinute, 0);
	}
...
//        if (_running)
//        {
//            return PFDataHelper.FormatString("任务{0}运行中,定时设置在每天的{1}时{2}分,下次执行的时间为{3}", HashId, BackupHour, BackupMinute, _nextRunTime.toString());
//        }
//        else
//        {
...
//    }
    ...
    public String GetStatusDescription()
    {
        if (_running.get())
        {
            return SGDataHelper.FormatString("任务{0}运行中,定时设置在每月的{1}日{2}时{3}分,每月条件满足时运行一次,下次执行的时间为{4}", HashId, BackupDay, BackupHour, BackupMinute, _nextRunTime.toString());
        }
        else
        {
