...
import static com.sellgirl.sgJavaHelper.express.PFFormulaItem.*;
 ...
 * 原理步骤:
 * 1.根据运算符号拆分字符串
 * 2.根据运算的mark和variable位置来匹配运算
 * 3.把指针往后移,在指针为起点往左往右找公式,取优先级高的;如果公式在指针右边,还要以公式使用到的每一位为起点往右找,看有没有更高优先级的公式
 * @author li
 *
 */
public class PFExpressHelper {
	...
	public static PFExpressHelper popular() {
				...
				new PFFormula(
						...
						(a,b,c)->{
							if((a!=null&&a.toString().charAt(0)=='"')
								...
								return SGDataHelper.ObjectToInt(a)+SGDataHelper.ObjectToInt(b);
							}
							},100
						)
				);
		...
		r.formula.add(
				new PFFormula(
						new PFFormulaItemCollection(variable(),mark("-"),variable()) ,
						(a,b,c)->SGDataHelper.ObjectToInt(a)-SGDataHelper.ObjectToInt(b),100
						)
				);
		//乘法
		r.formula.add(
				new PFFormula(
						new PFFormulaItemCollection(variable(),mark("*"),variable()) ,
						(a,b,c)->SGDataHelper.ObjectToInt(a)*SGDataHelper.ObjectToInt(b),50
						)
				);
		...
		r.formula.add(
				new PFFormula(
						...
						(a,b,c)->SGDataHelper.ObjectToInt(a),100
						)
				);
		return r;
	}
		...
		for(String i : marks) {
			...
			if(idx>-1) {
				...
				if(quotaIdx>-1&&quotaIdx<idx) {
					...
					if(s.substring(quotaIdx+1,idx).indexOf("\"")>-1) {//之前的判断有问题,截取的左边位置不正确--benjamin20210910
...
//		}
		//先找左边的公式
		for(PFFormula i :formula) {
//			if("[\"and\",".equals(i.format.get(0).mark)&&idx==2) {//测试用
//				String aa="aa";
//			}
//			if("[\"and\",".equals(i.format.get(0).mark)&&idx==4) {
//				String aa="aa";
//			}
			SGRef<Integer> lengthRef=new SGRef<Integer>(0); 
			if(i.matchRight(left,lengthRef)) {//匹配到运算
				if(lastOrder==-1||lastOrder>i.order) {
					r=i;
					lastOrder=i.order;
					//formulaPos=ImmutablePair.of(idx-i.format.size()+1, idx);//多参数之后,长度不是format
					formulaPos=ImmutablePair.of(idx-lengthRef.GetValue()+1, idx);
				}
			}
		}
		//往右找公式
		for(PFFormula i :formula) {
			SGRef<Integer> lengthRef=new SGRef<Integer>(0); 
			if(i.matchLeft(right,lengthRef)) {//匹配到运算
				if(lastOrder==-1||lastOrder>i.order) {
					r=i;
					lastOrder=i.order;
//					formulaPos=ImmutablePair.of(idx,idx+i.format.size()-1);//多参数之后,长度不是format
					formulaPos=ImmutablePair.of(idx,idx+lengthRef.GetValue()-1);
					bestIsRight=true;
				}
			}
		}
...
//			}
			...
			while(rightHasBest) {
				...
				for(int i=formulaPos.getLeft()+1;i<=formulaPos.getRight();i++) {
					...
					for(PFFormula j :formula) {
						SGRef<Integer> lengthRef=new SGRef<Integer>(0); 
						if(j.matchLeft(bRight,lengthRef)) {//匹配到运算
							if(lastOrder>j.order) {
								r=j;
								lastOrder=j.order;
								//formulaPos=ImmutablePair.of(formulaPos.getRight(),formulaPos.getRight()+j.format.size()-1);
								formulaPos=ImmutablePair.of(formulaPos.getRight(),formulaPos.getRight()+lengthRef.GetValue()-1);
								rightHasBest=true;
							}
						}
					}
...
//				}
				if(fMarkIdx>0) {//符号左边有变量
					String tmpV=tmpExpress.substring(0,fMarkIdx);
					if(!tmpV.equals(" ")) {//json字符串的逗号右边的空格不要当成变量--benjamin 20210903
						r.add(variable(tmpV.trim()));
					}
				}
...
//				PFFormulaItem n=variable(bestFormula.calculate(rr.GetValue().subList(formulaPos.GetValue().getLeft(),formulaPos.GetValue().getRight()+1)));//结束点不要设置,否则多参数时不完整
				PFFormulaItem n=variable(bestFormula.calculate(rr.GetValue().subList(formulaPos.GetValue().getLeft(),rr.GetValue().size())));
