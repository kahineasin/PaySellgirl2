...
/**
 * 尝试改写enum(感觉这种方式比PFEnumClass的方式更好,但未进一止测试稳定性,也未实现PFEnumClass的所有功能)(推荐还是用java原生的enum)
 * 
 * 使用方式:参考TestEnum001
 * 
 * @author 1011712002
 ...
 */
@Deprecated
public abstract class PFEnum2<E extends PFEnum2<E>> implements Comparable<E>, Serializable, IPFEnum {
	/**
		 * 
		 */
	private static final long serialVersionUID = 5941147702065383086L;
	/**
	 ...
	 *                an ordinal of zero).
	 */
	protected PFEnum2(String name, int ordinal) {
		this.name = name;
	 ...
	 *
	 * Enum constants are only comparable to other enum constants of the same enum
	 * type. The natural order implemented by this method is the order in which the
	 * constants are declared.
	 */
	public final int compareTo(E o) {
		PFEnum2<?> other = (PFEnum2<?>) o;
		PFEnum2<E> self = this;
...
//* @since 1.5
//*/
//public static <T extends PFEnum2<T>> T valueOf(Class<T> enumType,
//                                        String name) {
...
//}
	public static <T extends PFEnum2<T>> T valueOf(Class<T> enumType, String name) {
		try {
			return SGDataHelper.ObjectAs(enumType.getField(name).get(null));
		} catch (Exception e) {
		}
		return null;
	}
	public static <T extends IPFEnum> T EnumParseByInt(Class<T> cl,int v) {
		try {
