...
/**
 * 如果每个PFListenEmailTask都自己收邮件监听的话，那很浪费资源，应该用PFEmailListener一次性监听新邮件
 * 内嵌结构是为了达到以下目的：
 * 1.a不管在任何地方监听，新邮件都只接收一次
 * 2.云架构里，就算静态成员不共享也不互相影响（需要测试）
 * @author Administrator
 *
 */
public class PFEmailListener// implements IPFTask//, IDisposable
{
	private static PFEmailListenReceiver receiver=null;
	private static boolean isInited=false;
	//protected PFEmailListener() {}
	public static PFEmailListener init(PFEmailManager emailManager) {
		if(!isInited) {
			isInited=true;
			receiver=new PFEmailListenReceiver(emailManager);
			receiver.Start();
		}
		return new PFEmailListener();
	}
    public void OnReceiveNew(String hashId,Consumer<PFEmail> doConsumer,boolean onlyListenOnce
    		//,Function<PFEmail, Boolean> subjectMatch
    		)
    {
    	receiver.OnReceiveNew(hashId,doConsumer,onlyListenOnce//,subjectMatch
    			);
    }
    public void UnOnReceiveNew(String hashId)
    {
    	receiver.UnOnReceiveNew(hashId);
    }
    public static class PFEmailListenItem{
    	public Consumer<PFEmail> doAction=null; 
    	public boolean onlyListenOnce=false;
    }
	public static class PFEmailListenReceiver{
	    ...
	    //private List<Consumer<PFEmail>> newEmailAction=new ArrayList<Consumer<PFEmail>>();
	    //private PFKeyValueCollection<Consumer<PFEmail>> doConsumer=new PFKeyValueCollection<Consumer<PFEmail>>();
	    private PFKeyValueCollection<PFEmailListenItem> doConsumer=new PFKeyValueCollection<PFEmailListenItem>();
	    //private PFKeyValueCollection<Function<PFEmail, Boolean>> subjectMatch=new PFKeyValueCollection<Function<PFEmail, Boolean>>();
	    ...
	    private SGDate _lastListenTime;
...
//	        SubjectMatch = subjectMatch;
	        ...
	        _initTime=_lastListenTime = SGDate.Now();
	    ...
	    protected void OnReceiveNew(String hashId,Consumer<PFEmail> doConsumer,boolean onlyListenOnce
	    		//,Function<PFEmail, Boolean> subjectMatch
	    		)
	    {
//	    	this.doConsumer.Add(hashId,doConsumer);
	    	PFEmailListenItem item=new PFEmailListenItem();
	    	item.doAction=doConsumer;
	    	item.onlyListenOnce=onlyListenOnce;
	    	this.doConsumer.Add(hashId,item);
	    	//this.subjectMatch.Add(hashId,subjectMatch);
	    }
	    protected void UnOnReceiveNew(String hashId)
	    {
	    	this.doConsumer.Remove(hashId);
	    	//this.subjectMatch.Remove(hashId);
	    }
	    public void Start()
	    {
	        if (!_running)
...
//	    }
	    ...
	    public void StartThread()//Object ps)
	    {
			...
			int step=-1;//a只是为了检查报错时的位置(正常情况下是不用的,所以要检查为什么有时没报错误堆)
	        while (_running == true)
	        {
	            try
	            {
	                ...
	                {
	                    //为了检查26服务器上监听邮件失效的问题--benjamin todo
	                    SGDataHelper.WriteLocalTxt(SGDataHelper.FormatString("监听邮件的while的_emailManager.Connect_Click()执行成功，时间:{0}，邮件数：{1}", SGDate.Now().toString(), _emailManager.MessageCount), "PFListenEmailTask_while_Connect_Click.txt",LocalDataType.Deletable);
	                }
	                catch (Exception e) { }

	            	step=354;
	                SGDate newestMailTime = null;//记录最新邮件的时间
	                ...
	                for (int i = _emailManager.MessageCount; i > 0; i--)
	                {
	                    ...
	                    {
	                        //为了检查26服务器上监听邮件失效的问题--benjamin todo
	                        SGDataHelper.WriteLocalTxt(SGDataHelper.FormatString("监听邮件的while的_emailManager.Retrieve_Click()执行成功，时间:{0}，邮件数：{1}", SGDate.Now().toString(), _emailManager.MessageCount), "PFListenEmailTask_while_Retrieve_Click.txt",LocalDataType.Deletable);
	                    }
	                    catch (Exception e) { 
	                    ...
	                    SGDate emailDate=email.getSentDate()==null?null:new SGDate(email.getSentDate());
...
//	                    }
	                    if(emailDate != null)
	                    {
	                        ...
	                        else if (emailDate.compareTo(_lastListenTime)<=0 )//今天的邮件
	                        {
	                            ...
	                            if(SGDataHelper.ListAny(_emailAtToday, a->a.equals(emailRef.GetValue())))//今天的邮件要比对
	                            {
	                                //email.Dispose();
	                                email=null;
	                                //continue;
	                        ...
	                        {
	                            if(!SGDataHelper.ListAny(_emailAtToday,a -> a.equals(emailRef.GetValue())))
	                            {
	                                _emailAtToday.add(pfEmail);
	                            }
	                        }
	                    ...
	                    {
	                        isAnyReceiveFail = true;
//	                        //email.getSentDate()出现null的情况，原因不明--benjamin 20250208
                            email=null;
                            continue;
	                    }
	                	step=457;
...
//	                    for(PFKeyValue<Consumer<PFEmail>> a:doConsumer) {
//	                    	a.Value.accept(pfEmail);
////		                    Boolean b = false;
////		                    try
////		                    {
////		                        //b = SubjectMatch.apply(email);
////		                        b = this.subjectMatch.Get(a.Key).apply(pfEmail);
////		                    }
////		                    catch (Exception e)
////		                    {
////		                        newestMailTime = null;
...
////		                    }
//	                    }
	              	   for(int doIdx=doConsumer.size()-1;doIdx>=0;doIdx--) {
		              		 PFKeyValue<PFEmailListenItem> item=doConsumer.get(doIdx);
	              		   
		                	String log=SGDataHelper.FormatString(
		                			"任务{0}监听到新邮件{1}",item.Key, pfEmail.getSubject());
		                	System.out.println(log);
		              	   System.out.println("");
		              	   
	              		item.Value.doAction.accept(pfEmail);
	              		if(item.Value.onlyListenOnce) {
	              			doConsumer.remove(item);
	              		}
	              	   }
...
//	                		new Exception(PFDataHelper.FormatString("邮箱帐号权限报错: userId[{0}] pwd[{1}] \r\n error:{2}", _emailManager._userName,_emailManager._pwd,e)));
	                SGDataHelper.WriteErrors(Arrays.asList(e,new Exception(SGDataHelper.FormatString("邮箱帐号权限报错: userId[{0}] pwd[{1}] \r\n error:{2}", _emailManager._userName,_emailManager._pwd,e)))
	                		);
	                try {
						Thread.sleep(SGDataHelper.CheckMessageInterval());
