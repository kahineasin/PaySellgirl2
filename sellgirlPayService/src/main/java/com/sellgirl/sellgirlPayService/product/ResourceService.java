package com.sellgirl.sellgirlPayService.product;
...
//         protected IShopJdbc jdbc;
         protected ISGJdbc jdbc;
         public void setJdbc(ISGJdbc jdbc) {
        	 this.jdbc=jdbc;
         }

         /**
          * 查询一条书数据
          * 此方法没有在sgPayAdminCli中共用, 
          * 放在这里是为了测试Service被多种不同注入框架的app共用时的可能性
          */
         public SGDataTable GetResourceById(long id,ResourceType type)
         {
//     		ISGJdbc jdbc=JdbcHelper.GetShop();
