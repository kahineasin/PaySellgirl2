...
// 只有前三个字节是TAG才处理后面的字节
		if (tag.equalsIgnoreCase("TAG")) {
			...
			songName = new String(data, 3, 30,encode).trim();
			artist = new String(data, 33, 30,encode).trim();
			album = new String(data, 63, 30,encode).trim();
			year = new String(data, 93, 4,encode).trim();
			comment = new String(data, 97, 28,encode).trim();
			r1 = data[125];
			r2 = data[126];
			r3 = data[127];
		} else {
